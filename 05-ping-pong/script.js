// Create JS representation from the DOM  -  Crear representación JS del DOM
...
// Game Variables  -  Variables del Juego
...
function moveBall() {
    ...
    // Wall collision -  Colisión con las paredes
    if (ballY >= gameHeight - ball.clientHeight || ballY <= 0) {
        ballSpeedY = -ballSpeedY;
        playSound(wallSound);
    }

    // paddle 1 collision -  Colisión con la paleta 1
    if (
    ...
    // paddle 2 collision -  Colisión con la paleta 2
    if (
    ...
    // ball out of bounds -  Bola fuera de los límites
    if (ballX <= 0) {
        player2Score++;
        playSound(lossSound);
        updateScoreboard();
